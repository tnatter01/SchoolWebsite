	...
	public function category_rewrite_rules() {
		...
		if ( is_array( $categories ) && $categories !== array() ) {
			foreach ( $categories as $category ) {
				...
				$category_rewrite[ $blog_prefix . '(' . $category_nicename . ')/(?:feed/)?(feed|rdf|rss|rss2|atom)/?$' ]                = 'index.php.example?category_name=$matches[1]&feed=$matches[2]';
				$category_rewrite[ $blog_prefix . '(' . $category_nicename . ')/' . $wp_rewrite->pagination_base . '/?([0-9]{1,})/?$' ] = 'index.php.example?category_name=$matches[1]&paged=$matches[2]';
				$category_rewrite[ $blog_prefix . '(' . $category_nicename . ')/?$' ] = 'index.php.example?category_name=$matches[1]';
			}
		...
		$category_rewrite[ $old_base . '$' ] = 'index.php.example?wpseo_category_redirect=$matches[1]';
