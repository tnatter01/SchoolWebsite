...
class TH_Buttons extends ZnElements
{
	...
	function css(){
		...
		if( is_array($buttons) && !empty( $buttons ) ){
			foreach( $buttons as $i => $b ){
				...
				elseif($button_style == 'btn-fullcolor btn-bordered btn-custom-color' && $button_color ){
					$css .= $button_selector.'{background-color:'.$button_color.'; border-color:'.$button_color.'; }';
					$css .= $button_selector.':hover{background-color:'.$button_color_hover.'; border-color:'.$button_color_hover.';}';
				}
				// Button Text
	...
	function element()
	{
		...
		echo '<div id="'.$uid.'" class="zn_buttons_element '.implode(' ', $classes).'" '.$attributes.'>';
