							...
							if ( isset ( $options['sc_lp_image'] ) && ! empty ( $options['sc_lp_image'] ) )
							{
								echo '<div class="col-sm-10 col-sm-offset-1 col-md-7 col-md-offset-0">';
									echo '<div id="screenshot" class="sc__screenshot">';
									    ...
									    $customImageWidth = $this->opt( 'sc_lp_custom_width', null );
									    if( ! empty($customImageWidth) ) {
											$image = vt_resize( '', $options['sc_lp_image'], $customImageWidth, '', true );
                                        }
									    else {
											$image = vt_resize( '', $options[ 'sc_lp_image' ], '653', '390', true );
										}
		...
		$options = array(
			...
			'general' => array(
				...
				'options' => array(
					...
					array(
						'id'          => 'sc_lp_custom_width',
						'name'        => __( 'The custom width of the image', 'zn_framework'),
						'description' => __( 'Choose the desired width for the image.', 'zn_framework' ),
						'type'        => 'slider',
						'std'        => '653',
						'helpers'     => array(
							'min' => '100',
			...
			'help' => znpb_get_helptab( array(
				'video'   => sprintf( '%s', esc_url('https://my.hogash.com/video_category/kallyas-wordpress-theme/#Vh_jEQp5RTY') ),
				'docs'    => sprintf( '%s', esc_url('https://my.hogash.com/documentation/static-content-product-loupe-2/') ),
				'copy'    => $uid,
				'general' => true,
			)),
