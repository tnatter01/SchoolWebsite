...
class TH_FlexSlider extends ZnElements
{
	...
	function element()
	{
		...
		$show_thumbs = $this->opt('fs_show_thumbs', '0') == 1;
		...
		<div class="<?php echo implode(' ', $classes); ?>" <?php echo zn_get_element_attributes($options); ?>>
			...
			<div class="kl-slideshow-inner container kl-slideshow-safepadding">
				<div class="row">
					<div class="col-sm-12">
						<div class="zn_simple_slider_container slick--showOnMouseover <?php echo $fs_shadow;?> kl-flex--<?php echo $sliderStyle; ?>">
							<?php
								...
								$slick_attributes = array(
									...
									"easing" => 'easeOutExpo',
									"fade" => $this->opt('fs_transition','fade') == 'fade' || $this->opt('fs_transition','fade') == 'crossfade' ? true : false,
								);

								$has_nav = $this->opt('fs_nav', 'true') == 'true';
								if($has_nav){
								...
								// Dots
								if($this->opt('fs_bullets', 'true') == 'true' || $show_thumbs) {
							...
							<?php if( $this->opt('fs_bullets', 'true') == 'true' || $show_thumbs ){ ?>
