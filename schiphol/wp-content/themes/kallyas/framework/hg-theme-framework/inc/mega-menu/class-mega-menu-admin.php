...
// We need to load the default menu walker so we can extend it
	...
	*/
	function znfw_wp_admin_nav_menus_css($hook)
	{
		...
		wp_enqueue_style( 'znfw_wp_admin_nav_menus_css', ZNHGTFW()->getFwUrl( 'assets/css/admin_mega_menu.css' ) );
		...
		wp_localize_script('zn-mega-menu', 'ZnHgAdminMegaMenu', array(
            'popupTitle' => esc_html( __( 'Select image', 'zn_framework' ) ),
        ));
	}
	...
	function znfw_add_menu_button_fields( $item_id, $item, $depth, $args ){
		...
		<p class="field-enable-mega-menu description description-wide">
			<label for="enable-mega-menu-<?php echo $item_id; ?>">
				<input id="enable-mega-menu-<?php echo $item_id; ?>" type="checkbox" class="menu-item-checkbox znkl-mega-menu-enable" <?php echo $value; ?>
                       name="<?php echo $key; ?>[<?php echo $item_id; ?>]"
        ...
        <div class="field-add-image-mega-menu description description-wide <?php echo esc_attr( $cssClass );?>" data-ref="<?php echo esc_attr($dependencyImageID); ?>">
            ...
            <div class="zn-hg-image-upload-mega-menu-wrapper">
                    ...
                    <input type="button"
                           ...
                           value="<?php esc_html_e( 'Select image', 'zn_framework' );?>"
                           id="<?php echo esc_attr($dependencyImageID); ?>"/>
                    <input type="hidden"
                           class="zn-hg-bg-image-input-h"
                           name="<?php echo $key; ?>[<?php echo esc_attr($item_id); ?>]"
		...
		<?php
		// Smart area
		$title = __( 'Use a smart area ?' , 'zn_framework' );
        ...
        <p class="field-enable-mega-menu-smart-area description description-wide">
            <label for="enable-mega-menu-smart-area-<?php echo $item_id; ?>">
				<?php echo $title; ?>
            </label>
            <select id="enable-mega-menu-smart-area-<?php echo $item_id; ?>" class="znkl-mega-menu-smart-area" name="<?php echo $option_key; ?>[<?php echo $item_id; ?>]">
				<?php
